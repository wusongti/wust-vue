(function ($) {
    ...
    $.fn.treeTable = function (opts) {
        ...
        $treeTable.unbind('click').click(function (event) {
            var $target = $($(event.target).parents('tr')[0]).children("td:first-child").children("span:last-child");
            ...
            if ($target.attr('arrow')) {
              ...
              if (className == css['AN'] + ' ' + css['HLO'] || className == css['AN'] + ' ' + css['HO']) {
                ...
                shut(id);
                return false;
              } else if (className.indexOf('_shut') != -1) {
        ...
        function formatNode(tr) {
            ...
            if($td.find('span.prev_span').length < 1){
