  ...
  export default {
    ...
    methods: {
      ...
      openSubMenu: function (event,menuId) {
          var className = event.target.parentElement.lastElementChild.className;
          if(className == 'menu-arrow arrow_carrot-down arrow_carrot-right'){ // 展开子菜单
            // 先关闭已经打开的菜单的样式
            var arrowCarrotDowns = document.querySelectorAll('.menu-arrow.arrow_carrot-down');
            if(arrowCarrotDowns != null && arrowCarrotDowns.length > 0){
              for(var i=0;i<arrowCarrotDowns.length;i++){
                var arrowCarrotDown = arrowCarrotDowns[i];
                arrowCarrotDown.className = 'menu-arrow arrow_carrot-down arrow_carrot-right';
              }
            }

            // 先关闭已经打开的子菜单的样式
            var subBlocks = document.querySelectorAll('.sub');
            ...
            /*var classNameActives = document.querySelectorAll('li.active');
            if(classNameActives != null && classNameActives.length > 0){
              var classNameActive = classNameActives[0];
              classNameActive.className = '';
            }

            var liClassName = event.target.parentElement.className;
